make_graph_out <- function(out, bw, x_low = 0, x_high, include_legend = TRUE, add_smooth = FALSE, point_estimate = NA){
  ...
  out_df$colours <- ifelse(out_df$out < 0.03, "Population risk", ifelse(out_df$out < 0.08, "Moderate risk", "High risk"))
  
  plot <- ggplot(out_df, aes(x = out)) + geom_histogram(data = out_df, binwidth = bw, boundary = bw, aes(fill = colours)) + 
    #labs(x = "10-year risk from 40", y = "Count", fill = "") + scale_fill_manual(values = c("Population risk" = "lightskyblue", "Moderate risk" = "orange", "High risk" = "red")) + 
    labs(x = "10-year risk from 40", y = "Count", fill = "") + scale_fill_manual(values = c("Population risk" = "royalblue", "Moderate risk" = "orange", "High risk" = "red")) + 
    ...
    scale_y_continuous(expand = c(0, 0), limits = c(0, NA)) + scale_x_continuous(expand = c(0, 0), limits = c(x_low,x_high))
